...
export default function CricketControl() {
    ...
    <div style={{ minHeight: "100vh", background: "#FEFDF8", fontFamily: "Georgia, serif" }}>
      <section style={{ background: `linear-gradient(160deg, ${DARK}, #1a3a2a)`, color: "#fff", padding: "72px 40px 64px", position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: "clamp(34px,4.5vw,58px)", fontWeight: 400, lineHeight: 1.1, marginBottom: 6 }}>Cricket Control</h1>
          <h1 style={{ fontSize: "clamp(34px,4.5vw,58px)", fontWeight: 700, lineHeight: 1.1, marginBottom: 22, color: GOLD }}>Alabama Specialists.</h1>
          <p style={{ fontSize: 16, color: "rgba(255,255,255,0.7)", lineHeight: 1.75, maxWidth: 560, marginBottom: 32, ...sf }}>House crickets chirp all night from walls and garages. Camel crickets invade damp basements in large numbers. Both enter Alabama homes seeking shelter, especially as temperatures drop in fall.</p>
      ...
      <section style={{ padding: "72px 40px", background: "#f7f9f7" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <h2 style={{ fontSize: 28, fontWeight: 400, color: DARK, textAlign: "center", marginBottom: 40 }}>Cricket in <em style={{ color: G }}>Alabama</em></h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            {[
              { icon: "🔍", title: "Two Types", desc: "House crickets: tan, loud chirping. Camel crickets: brown, humpbacked, silent, jump erratically when startled" },
              { icon: "🔍", title: "Light Attraction", desc: "Strongly attracted to exterior lighting. Bright porch lights draw them directly to entry points" },
              { icon: "🔍", title: "Peak Season", desc: "Heaviest indoor invasion September through November as temperatures drop" },
              { icon: "🔍", title: "Fabric Damage", desc: "Feed on cotton, wool, silk, and linen. Can damage clothing, curtains, and upholstery" },
            ].map((item, i) => (
      ...
      <section style={{ padding: "64px 40px", background: `linear-gradient(160deg, ${DARK}, #1a3a2a)`, color: "#fff" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <h2 style={{ fontSize: 24, fontWeight: 400, marginBottom: 20 }}>How We Treat <em style={{ color: GOLD }}>Cricket</em></h2>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.7)", lineHeight: 1.8, ...sf }}>Exterior perimeter barrier stops crickets before entry. Granular bait around the foundation eliminates crickets in landscape beds. Interior treatment targets garages, basements, and utility rooms. Reducing outdoor lighting near doors helps — crickets are strongly attracted to light.</p>
        </div>
      </section>

      <section style={{ background: "#FEFDF8", padding: "56px 40px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <h2 style={{ fontSize: 26, fontWeight: 400, color: DARK, marginBottom: 28, textAlign: "center" }}>Cricket FAQ</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              { q: "Why do I hear chirping in my walls?", a: "Male house crickets chirp to attract mates. They entered through foundation cracks or utility penetrations." },
              { q: "Are camel crickets dangerous?", a: "No. They do not bite or chirp. But they appear in large numbers and jump unpredictably. They can damage stored fabrics." },
              { q: "Why so many crickets near my front door?", a: "Your porch light is attracting them. Switch to yellow or sodium vapor bulbs which attract fewer insects." },
            ].map((item, i) => (
      ...
      <section style={{ background: `linear-gradient(160deg, ${G}, #0d3a1a)`, padding: "56px 40px", textAlign: "center", color: "#fff" }}>
        <div style={{ maxWidth: 520, margin: "0 auto" }}>
          <h2 style={{ fontSize: 32, fontWeight: 400, lineHeight: 1.2, marginBottom: 12 }}>Stop Crickets <em>For Good</em></h2>
